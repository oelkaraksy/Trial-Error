...
// Quick-select range buttons (6M / 1Y / 2Y)
document.querySelectorAll('.range-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    // Update slider value and fire input event to trigger chart redraw
    range.value = months;
    range.dispatchEvent(new Event('input'));

    // Update active state within the same button group
    const group = btn.closest('.range-btns');
    group.querySelectorAll('.range-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
  });
});

// Also sync active button state when slider is moved manually
['annual-range', 'monthly-range'].forEach(id => {
  const range = document.getElementById(id);
  if (!range) return;
  range.addEventListener('input', () => {
    const months = parseInt(range.value);
    const group = range.closest('.controls-bottom').querySelector('.range-btns');
